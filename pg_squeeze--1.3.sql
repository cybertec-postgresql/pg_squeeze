...
-- Create a new task for each table that should be checked right now according
-- to the schedule.
--
-- Since this function is executed by the "scheduler worker", it does not
-- check free space because the check can take some time. Any unnecessary
-- delays should be avoided so that we don't miss the next schedule of any
-- table. The "squeeze worker" will check the free space and possibly delete
-- the task.
...
-- The "squeeze worker" calls this function to check if tables having a new
-- task should really be squeezed. Task status is set either to 'ready' (i.e
-- ready to be squeezed) or 'processed' if there squeeze criteria are not met
...
--
-- By adjusting last_task_finished make VACUUM necessary before the next task
...
-- Like above, but do not update last_task_finished.
